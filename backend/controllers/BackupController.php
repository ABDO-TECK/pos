...
class BackupController extends Controller {
    ...
    public function restore(): void {
        if (empty($_FILES['sql_file']) || (int) ($_FILES['sql_file']['error'] ?? 0) !== UPLOAD_ERR_OK) {
            Response::error('لم يتم رفع الملف أو فشل الرفع', 400);
        }

        $file = $_FILES['sql_file'];
        $name = (string) ($file['name'] ?? '');
        if (!str_ends_with(strtolower($name), '.sql')) {
            Response::error('يجب أن يكون الملف بصيغة .sql', 400);
        }

        $maxBytes = 50 * 1024 * 1024; // 50 MB
        if ((int) ($file['size'] ?? 0) > $maxBytes) {
            Response::error('حجم الملف يتجاوز الحد المسموح (50 ميجابايت)', 400);
        }

        $content = file_get_contents($file['tmp_name']);
        if ($content === false || strlen($content) < 30) {
            Response::error('الملف فارغ أو غير قابل للقراءة', 400);
        }
        ...
        if (!preg_match('/\b(DROP\s+TABLE|CREATE\s+TABLE|INSERT\s+INTO)\b/is', $content)) {
            Response::error('محتوى الملف لا يبدو ملف SQL صالحاً لقاعدة البيانات', 400);
        }
        ...
        if (preg_match('/\b(OUTFILE|DUMPFILE|LOAD_FILE|INTO\s+OUTFILE)\b/is', $content)) {
            Response::error('الملف يحتوي على أوامر غير مسموحة', 400);
        }

        $mysqli = @new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
        if ($mysqli->connect_errno) {
            Response::serverError('فشل الاتصال بقاعدة البيانات: ' . $mysqli->connect_error);
        }
        ...
        if (!$mysqli->multi_query($content)) {
            $err = $mysqli->error;
            $mysqli->close();
            Response::error('فشل تنفيذ الاستعادة: ' . $err, 500);
        }
        ...
        do {
            ...
            if (!$mysqli->next_result()) {
                $err = $mysqli->error;
                $mysqli->close();
                Response::error('فشل أثناء الاستعادة: ' . $err, 500);
            }
        } while (true);
        ...
        Response::success(null, 'تمت استعادة قاعدة البيانات بنجاح');
    }
}
